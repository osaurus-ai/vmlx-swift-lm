...
- **Iter 4 (TBD)** — Phase 1 late kickoff: `TokenEmbedderModel` protocol + Qwen3 conformance (exposes `embed(_:)` + `projectToLogits(_:)`). `SpecDecRuntimeLinear.run(_:)` ports z-lab's `spec_generate` — prefill, draft→verify→accept loop, acceptance-length tracking. `DFlashLinearArgs` / `DFlashLinearResult` public types. `SpecDecRuntime` actor delegates to the stateless runtime. 4/4 `DFlashLinearRuntimeSmokeTests` green on random-weight tiny models (proves plumbing end-to-end). Simplification vs Python: re-prefills target each round instead of cropping KV cache — correct but slow; iter 6 will optimise via `CacheCoordinator`. Byte-parity vs real-weight autoregressive still pending iter 5 — needs matching drafter+target checkpoint load path.
