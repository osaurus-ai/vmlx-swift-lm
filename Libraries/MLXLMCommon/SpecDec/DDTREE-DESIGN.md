...
- **Iter 13 (TBD)** — Phase 5 JANG capability stamp. `JangCapabilities` grows four new fields: `draftStrategy` / `drafterPath` / `branchingBudget` / `blockSize` decoded from `jang_config.json["capabilities"]`. New `ParserResolution.draftStrategy(capabilities:modelDirectory:)` facade maps the stamp → concrete `DraftStrategy` enum with the drafter path resolved relative to the model snapshot root. 12/12 `JANGSpecDecCapabilityTests` green — field presence, nil handling, dflash/ddtree/none/unknown stamps, default branchingBudget=32, missing blockSize/drafterPath gating, and a full JSON decoder round-trip test that writes jang_config.json to a temp dir then reads it back through `JangLoader.loadConfig`. Closes the `JANGSpecDecCapabilityTests.swift` row in the test matrix. Next iter: factories (LLMModelFactory/VLMModelFactory) can auto-populate `ModelConfiguration.draftStrategy` from the stamp so osaurus gets zero-config SpecDec on JANG-stamped models.
