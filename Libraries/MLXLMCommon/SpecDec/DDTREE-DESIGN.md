...
- **Iter 7 (TBD)** — Phase 2 middle. `TreeCompile.compile(tree:rootTokenID:prefixLen:)` ports `compile.py`. Produces `CompiledTree` with: `input_ids` uint32 `(1, T)` = [rootToken, node tokens]; `position_ids` int32 = prefix_len + node depths; `attention_mask` float32 `(1, 1, T, T)` with 0.0/-inf from tree visibility; `dfs_order` + `inv_dfs_order` via `TreeBuilder.computeDfsOrder`; `depths` [0, ...node_depths], `parents` + `treeSize` pass-through. 8/8 `DDTreeCompileTests` green: ordering, position-offset arithmetic, mask ancestor-only pattern, DFS permutation invariant, empty-tree edge case, prefix_len propagation. Next iter: `TreeVerify.verifyForward(target:compiled:cache:prefixLen:)` — the tree-verify forward pass with the combined `(1, 1, T, prefix_len + T)` attention mask + per-token RoPE.
