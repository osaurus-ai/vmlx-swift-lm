...
- **Iter 9 (TBD)** — **End-to-end DDTree byte-parity proven.** `SpecDecRuntimeDDTree.run(_:)` wires the full tree pipeline into a decode loop that mirrors `SpecDecRuntimeLinear.run` structure: prefill → draft → tree build → tree compile → tree verify → `followVerifiedTree` → commit accepted path + bonus → re-prefill target for hidden states (v1 slow path). `DDTreeArgs` + `DDTreeResult` public types with `branchingBudget` parameter. 4/4 `DDTreeEndToEndTests` green — DDTree output byte-identical to greedy AR across multiple prompts, branching budgets (1, 2, 4, 6, 8), seeds. Phase 2 correctness is now complete. The tree `follow_verified_tree` walker only accepts nodes that match target argmax, so committed sequence = AR sequence regardless of drafter proposals or tree shape. Iter 10 next: single-forward tree-verify optimisation (combined attention mask + per-token RoPE) for the speedup metric; real-checkpoint benchmark follows.
