...
- **Iter 10 (TBD)** — **streaming integration landed.** `SpecDecStream.streamDflashLinear` + `streamDDTree` wrap the runtime functions in an `AsyncStream<Generation>` that emits the same `.chunk(String)` + `.toolCall(ToolCall)` + `.info(GenerateCompletionInfo)` events as the non-speculative `Evaluate.generate` path. Both `SpecDecRuntimeLinear.run` and `SpecDecRuntimeDDTree.run` now accept an optional `onCommitted: ([Int32]) -> Void` callback fired after each decode round's commit (additive, preserves non-streaming callers byte-identically). Streaming pipes committed tokens through `NaiveStreamingDetokenizer` → `ReasoningParser` (drops `<think>`) → `ToolCallProcessor` → `.chunk`. New `SpecDecDrafterResolver` actor caches loaded drafter checkpoints by path and resolves a `DraftStrategy` to `(model, targetBlockIDs, maskTokenID)`. 2/2 `SpecDecStreamTests` green: streamed chunks decode back to same tokens as bulk result. Phase 4 next: thread `DraftStrategy` through `Evaluate.generate` + `BatchEngine.generate` for zero-API-churn osaurus integration.
