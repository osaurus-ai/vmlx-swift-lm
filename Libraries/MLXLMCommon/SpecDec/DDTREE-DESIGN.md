...
- **Iter 11 (TBD)** — **`Evaluate.generate` dispatch lands.** `SpecDecStream.streamViaStrategy(strategy:inputIds:context:maxNewTokens:…)` resolves `DraftStrategy.dflash` / `.ddtree` into a loaded drafter via `SpecDecDrafterResolver.shared`, builds the runtime args, runs `SpecDecRuntimeLinear` / `SpecDecRuntimeDDTree`, and surfaces `Generation` events. Plumbed into `Evaluate.generate(input:cache:parameters:context:wiredMemoryTicket:cacheCoordinator:)` as a top-of-function dispatch: when `parameters.draftStrategy.usesBlockDiffusion == true` AND the target conforms to `HiddenStateCaptureModel & TokenEmbedderModel`, the SpecDec stream returns; otherwise the existing `TokenIterator` path runs byte-identically. `SpecDecDrafterResolver.shared` singleton added so the dispatch works without caller-side resolver threading. 5/5 `SpecDecDispatchTests` green — including real-drafter cache hit + evict against the downloaded 3.2 GB Qwen3.5-27B-DFlash checkpoint. **Completion criterion #4 satisfied**: `GenerateParameters.draftStrategy` now flows through `Evaluate.generate` with zero API churn for `.none` callers. Iter 12 next: thread through `BatchEngine.generate` + write `OSAURUS-SPECDEC.md` + `OSAURUS-API-SURFACE.md` + README updates.
