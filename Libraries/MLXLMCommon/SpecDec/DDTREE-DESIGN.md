...
- **Iter 8 (TBD)** — Phase 2 verify-v1. `TreeVerify.verifyForward(target:compiled:prefixTokens:captureLayerIDs:)` lands as a **correct but slow** attention-only port. For each tree node `i`, walks `parents[]` back to root, builds `(prefix + path_to_node_i)`, runs target forward, takes argmax of last-position logits → `posteriorTokens[i]`. O(N) forwards per verify round — zero speedup vs AR, but pins the algorithm byte-for-byte. `TreeVerifyResult` + `RecurrentSnapshot` structs preserved from Phase 0 stub. 4/4 `DDTreeVerifyTests` green: linear tree (budget=1) matches greedy AR, branching tree per-node posterior matches direct-path argmax, empty tree root-only, logits shape `(1, treeSize, vocab)`. The single-forward optimisation (combined `(1, 1, T, prefix_len+T)` attention mask + per-token RoPE — where the real DDTree speedup lives) is iter 10 work requiring Qwen3-side model surgery. Iter 9 next: end-to-end DDTree byte-parity test that wires `SpecDecRuntime` → `TreeBuilder` → `TreeCompile` → `TreeVerify` → `followVerifiedTree` into a complete decode loop.
