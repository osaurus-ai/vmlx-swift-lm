...
- **Iter 6 (TBD)** — Phase 2 kickoff. Port of `humanrouter/ddtree-mlx/tree.py` → `TreeBuilder.swift`. Includes: minimal `_MinHeap` (binary heap with lex-compared rank-tuple tiebreak), `buildFromTopK` (Algorithm 1 best-first heap), `build(draftLogits:budget:)` (argSort + log-softmax → buildFromTopK), `followVerifiedTree`, `computeDfsOrder`, `buildVisibilityMatrix`. 11/11 `DDTreeBuilderTests` green against hand-traced reference outputs: empty/flat/branching trees, budget=1 early-exit, visibility ancestor-only invariant, follow walks (mismatch / full path / immediate miss), DFS order. Next iter: port `compile.py` → `TreeCompile.compile(tree:rootTokenID:prefixLen:)` (tree → MLX tensors: input_ids, position_ids, attention_mask, dfs_order). Real-checkpoint DFlash speedup measurement pushed to iter 8+ after Phase 2 completes.
