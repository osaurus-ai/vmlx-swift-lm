...
- **Iter 5 (TBD)** — **byte-parity proven.** `DFlashLinearByteParityTests` asserts `SpecDecRuntimeLinear.run` produces output byte-identical to greedy autoregressive decode across multiple prompt lengths with seeded random-weight Qwen3. Greedy temp=0 invariant holds regardless of drafter quality — the test doesn't need real checkpoints. This pins the single strongest correctness contract (hard rule #1 in the ralph brief). 2/2 byte-parity tests green. Iter 6 will add real-checkpoint end-to-end verification + KV cache rollback for speedup measurement.
