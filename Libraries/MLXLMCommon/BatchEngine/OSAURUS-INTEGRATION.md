...
| **`OMNI-OSAURUS-HOOKUP.md`** | **2026-04-28 Nemotron-3-Nano-Omni multimodal integration guide.** Bundle detection, four-tower wrapper (LLM + RADIO + Parakeet + projectors), image/video/audio splice contracts, hybrid Mamba/Attn cache topology, TQ KV interaction, EVS pruning, JANGTQ × all-three-quants routing, cross-VLM `VLMVideoUtils.swift` shared library, the `LMInput.audio` open seam + workaround, BatchEngine multimodal-prefill open seam + workaround, full migration checklist. Read this if osaurus is wiring any omni bundle. |
